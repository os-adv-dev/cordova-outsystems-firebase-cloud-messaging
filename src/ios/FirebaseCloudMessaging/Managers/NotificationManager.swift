...
public protocol NotificationManagerProtocol {
    ...
    /// - Returns: A boolean indicating the success of the operation or an error, in case of failure.
    func sendLocalNotification(title: String, body: String, badge: Int, sound: String?) async -> Result<Bool, Error>
}
...
public final class NotificationManager: NSObject {
    ...
    private let localNotificationWrapper: OSLCNOActionDelegate
    
    /// Constructor method.
    /// - Parameters:
    ///   - coreDataManager: Manages Core Data related activites.
    ///   - notificationCenter: Manages Notification related activities.
    init(coreDataManager: CoreDataManager, notificationCenter: UNUserNotificationCenterProtocol, localNotificationWrapper: OSLCNOActionDelegate) {
        self.coreDataManager = coreDataManager
        self.notificationCenter = notificationCenter
        self.localNotificationWrapper = localNotificationWrapper
        super.init()
    }
    ...
    public convenience init(coreDataManager: CoreDataManager = CoreDataManager()) {
        ...
        let localNotificationWrapper = OSLCNOFactory.createUNWrapper()
        self.init(
            coreDataManager: coreDataManager, notificationCenter: notificationCenter, localNotificationWrapper: localNotificationWrapper
        )
    }
...
extension NotificationManager: NotificationManagerProtocol {
    public func sendLocalNotification(title: String, body: String, badge: Int, sound: String?) async -> Result<Bool, Error> {
        do {
            let firstButton = OSLCNOActionButton(id: "FirstButton", label: "Buy now", icon: "cart", type: .standard, event: .apiPost, processData: "https://facebook.com")
            let secondButton = OSLCNOActionButton(id: "SecondButton", label: "More details", icon: "magnifyingglass", type: .textInput(placeholder: "Insert text here..."), event: .apiPost, processData: "https://www.google.com/search?q={textInputPlaceholder}")
            let thirdButton = OSLCNOActionButton(id: "ThirdButton", label: "Third", icon: nil, type: .destructive, event: .apiPost, processData: "https://linkedin.com")
            
            try await self.localNotificationWrapper.triggerNotification(with: title, body, badge, sound, and: [firstButton, secondButton, thirdButton])
            return .success(true)
        } catch {
            return .failure(error)
        }
