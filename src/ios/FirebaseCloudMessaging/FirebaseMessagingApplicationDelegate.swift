...
extension FirebaseMessagingApplicationDelegate: UNUserNotificationCenterDelegate {
    ...
    public func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        if let userInfo = response.notification.request.content.userInfo as? NotificationDictionary {
            ...
            } else if var redirect = userInfo[response.actionIdentifier] as? String {
                if let textInputResponse = response as? UNTextInputNotificationResponse, redirect.contains("{textInputPlaceholder}") {
                    redirect = redirect.replacingOccurrences(of: "{textInputPlaceholder}", with: textInputResponse.userText)
                }
